...
Processing steps
----------------
1. Download raw ACS PUMS data into data/raw/ (explicit root_dir; no files are written to the user's home directory).
2. Normalize the raw DataFrame: rename RELSHIPP -> RELP when present. The Census Bureau renamed the household-relationship variable in 2019, but the folktables BasicProblem always references "RELP".
3. Build the ACSIncome task with a configurable income threshold and extract the feature matrix, label vector, and sensitive-attribute group via df_to_pandas().
4. Categorize all feature columns:
    - AGEP: binned into six career-stage bands
    - WKHP: binned into six work-hours bands
    - All remaining categorical columns decoded to human-readable string labels using the ACS PUMS code maps in constants.py. Result columns use pd.Categorical to reduce memory footprint.
...
------------------------
- Per-state download: ThreadPoolExecutor (I/O-bound; one HTTP request per state file, all independent).
- Column categorization: ThreadPoolExecutor (independent per-column transforms; NumPy-backed pandas operations release the GIL, enabling true concurrency).
- CSV write (split): two threads write train and test files simultaneously.
- Multi-year execution: ProcessPoolExecutor in main.py (CPU-bound; each year is a fully independent workload that bypasses the GIL via separate processes).
"""
...
def normalize_raw_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Background
    ----------
    The Census Bureau renamed the household-relationship column between survey years: raw PUMS files use "RELP" for years <= 2018 and "RELSHIPP" for years >= 2019.  The official folktables source (acs.py) always references "RELP" in the BasicProblem feature list but does not perform the rename automatically.  Without this normalization step, df_to_pandas() raises a KeyError for any 2019-2024 dataset.
    ...
    Returns
    -------
    DataFrame with the household-relationship column consistently named "RELP", regardless of the original survey year.
    """
...
def download_data(
    ...
    Single-state and all-states requests use the direct ACSDataSource path to avoid thread-pool overhead.  Multi-state requests are parallelized with a ThreadPoolExecutor: each state corresponds to a separate HTTP download, making this operation I/O-bound and therefore well-suited to thread-level concurrency.
...
def _build_acs_raw_task() -> folktables.BasicProblem:
    """
    Return an ACSIncome-equivalent task that keeps PINCP as a raw continuous value (no binarisation).

    Used internally to extract raw income before applying the dead zone filter and then binarising manually.
    """
...
def build_acs_income_task(threshold: float) -> folktables.BasicProblem:
    """
    Return an ACSIncome-equivalent BasicProblem with a configurable income threshold.

    The task predicts whether an individual's annual personal income (PINCP) exceeds `threshold` dollars.  All other parameters mirror the official ACSIncome definition in folktables/acs.py.
...
def _map_column(
    ...
    Codes absent from `mapping` are replaced by `fallback`. The result is stored as pd.Categorical to reduce memory usage and accelerate downstream groupby and value_counts operations.

    Uses an integer-keyed mapping internally: this avoids the expensive .astype(str) step which creates ~1M Python string objects on large datasets. Integer hashing is also faster than string hashing inside pandas .map().
    """
...
def _categorize_occp(series: pd.Series) -> pd.Categorical:
    ...
    Uses vectorised np.searchsorted on the pre-computed lower-bound array instead of a per-row Python function call - ~10* faster on 1M+ rows.
    ...
    # Vectorised range lookup: for each code, find the candidate range index via searchsorted, then verify the code falls within the upper bound of that range.
    ...
    # searchsorted with side='right' gives the insertion point; subtract 1 to get the index of the range whose lower bound is <= code.
    idx = np.searchsorted(lower_bounds, codes, side="right") - 1

    # Vectorised label assignment via fancy indexing (no Python loop). Build a label lookup with OCCP_FALLBACK at position -1 and len(ranges).
...
def categorize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Column-level transforms are fully independent of one another and are therefore dispatched to a ThreadPoolExecutor. The underlying NumPy array operations (astype, pd.cut, Series.map) release the GIL, enabling genuine concurrency across threads.
...
def build_dataset_stem(
    ...
    This function is the single source of truth for filename construction. It is also imported by main.py (_infer_split_paths) to ensure consistency.
    """
...
def create_dataset(
    ...
    horizon: Survey horizon -- "1-Year" or "5-Year".
    survey: Survey unit -- "person" or "household".
    states: Two-letter state codes to include, or None for all states in USA_STATES.
    threshold: Annual personal income threshold in U.S. dollars. The binary target label is 1 if PINCP > threshold.
    random_seed: Random seed for the stratified 80/20 train/test split.
    data_dir: Root output directory.  Raw PUMS files are cached in <data_dir>/raw/; processed CSVs are written to <data_dir>/.
    keep_columns: Feature columns retained in the output CSV. None (default) retains all feature columns. Pass a list to keep only specific columns.
    states_label: Optional group/region name used in the output filename instead of individual state codes (e.g. "northeast").If None the sorted state codes are used.
    margin: Dead zone half-width in dollars.  Individuals with PINCP in [threshold - margin, threshold + margin] are excluded from the dataset.  0 = disabled.
    ...
    """
    # Resolve keep_columns: None -> all columns (no filtering). When a list is provided, sort for consistent filename tags.
    ...
    # Reconstruct the expected output filename using build_dataset_stem(). If all three files exist, load and return them directly, skipping download + encoding entirely.
    ...
    # Use the raw task (no binarisation) so we can apply the dead zone filter before binarising the target.
    ...
    # Log the number of missing values replaced by the postprocess (np.nan_to_num converts NaN -> -1 in folktables).
    ...
    # Exclude individuals whose income falls within +/-margin of the threshold.  These are ambiguous cases where the binarised label depends on noise rather than structural differences in features. The margin is derived from the ACS Margin of Error for median family income, propagated through the Pew upper-income formula.
    if margin > 0:
        ...
        logger.info(
            ...
            lower, upper, n_excluded, n_before, n_excluded / max(n_before, 1) * 100,
        )
    ...
    # File name encodes every parameter that affects the dataset content, so runs with different configurations never overwrite each other.
    #
    # Pattern:
