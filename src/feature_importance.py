...
Adapted from:
  Alfeo et al. (2023) "From local counterfactuals to global feature importance: efficient, robust, and model-agnostic explanations for brain connectivity networks" Computer Methods and Programs in Biomedicine 236, 107550.

How the original paper uses data
---------------------------------
The paper states that BoCSoR operates on the training set.  In practice the training set serves as the *starting pool; the actual counterfactuals and the instances used for relevance testing are synthetic:

1. Boundary instances are selected from the training set (percentile filter on inter-class distance): these are real training rows.
2. For each boundary instance, k nearest neighbours of the opposite class are retrieved from the training set: these are also real.
3. Intermediate points are generated via np.linspace between the boundary instance and each neighbour: these are **synthetic**, not in any dataset.
4. The first synthetic midpoint classified as the opposite class becomes the closestCF: a synthetic point near the decision boundary.
5. Feature substitution on closestCF creates yet another synthetic instance, on which model.predict() is called: the model is thus probed on an instance it has never seen during training.

The model is therefore queried on synthetic, unseen instances to explore its decision surface.  This is functionally equivalent to probing with test data, except the probe points are constructed strategically near the boundary rather than sampled randomly.

Adaptations for fully-categorical data
---------------------------------------
* Boundary instances and the opposite-class pool come from the TRAINING SET, exactly as in the paper.  The test set is used only to compute a held-out accuracy estimate.

* No interpolation of intermediate points (not meaningful for categorical features).  Instead of generating synthetic midpoints (Algorithm 1 of the paper), counterfactuals are the K nearest neighbours of the opposite class in hybrid-encoded Manhattan space **that differ in at least one feature (distance > 0).  Cross-class duplicates (identical feature vectors, different label) are skipped by over-querying the NN index and filtering. This guarantees every counterfactual has >= 1 feature to substitute, producing clean itemsets for downstream ARM (stages 3-4).

* Feature substitution (Algorithm 2) creates synthetic instances just as in the paper: each modified counterfactual (one feature swapped back to the original value) is an instance that likely does not exist in the training set. model.predict() is called on these synthetic instances to determine which features are relevant: again probing the model on unseen data.

* Hybrid encoding (new):
- Ordinal columns (AGEP, SCHL, WKHP, ...): rank-based encoding normalised per-column via min-max to [0, 1].
- Nominal columns (all others): one-hot encoding divided by 2. Within a single nominal column two samples either share the same category (distance 0) or differ (raw Hamming distance 2, divided by 2 = 1.0, computed as Manhattan on the *0.5 encoded bits). Both groups therefore contribute values in [0, 1] per original column, making ordinal and nominal columns commensurable.

* Hybrid Manhattan distance (raw sum, no global normalisation): dist(a,b) = sum_i |enc_i(a) - enc_i(b)| where enc_i  in  [0, 1] for every encoded column i. A single nominal
feature change contributes 1.0; a single ordinal step contributes 1/(n_levels - 1).  No division by n_cols -- the sum is interpretable directly as "how many feature-equivalent changes apart".

* Multi-k evaluation via --k: Single value K  -> auto-expanded to all odd integers 1..K (plus K if even). --k 11 -> [1, 3, 5, 7, 9, 11] Multiple values -> used as-is.  --k 1 5 11 -> [1, 5, 11]

* Itemset output: one row per boundary instance per k value.  All relevant features are on the same row.  Columns: k_value, instance_index, features (space-separated names), itemset (space-separated FEATURE=value tokens, ARM-ready).  One combined file plus one file per k value.

* Two BoCSoR indices (computed per k):

1. Per-CF label-level index (bocsor_label_importance.csv): For each feature LABEL, counts how many times it is relevant across
ALL (boundary_instance, counterfactual) swap-and-restore tests, divided by the total number of counterfactuals tested. BoCSoR_label_k(SCHL) = n_times_SCHL_relevant / n_total_CFs. This distinguishes a feature relevant for every CF (strong signal) from one relevant for a single distant CF (weak signal).

2. Per-CF value-level index (bocsor_value_importance.csv): Same as above but keyed on LABEL=value tokens: BoCSoR_value_k(SCHL=Bachelors) = n_times_relevant / n_total_CFs. The old union-based index (feature_importance.csv) is retained for backward compatibility. It counts boundary instances (not CFs) and uses the union of relevant features across all k CFs per instance.

Parallelism and performance
----------------------------
* Precomputed NN query: the brute-force k-NN query (the most expensive step) is executed ONCE with k_query sized for the maximum k value.  The resulting (distances, indices) arrays are shared across all per-k runs, eliminating len(k_values)-1 redundant O(N * M) distance computations.
* Pre-allocated hybrid encoding: encode_hybrid() pre-allocates one contiguous output array via n_encoded_cols() and writes columns in-place, instead of building ~150 small arrays and concatenating.
* ThreadPoolExecutor for boundary chunk processing: CatBoost.predict() and MLPClassifier.predict() are implemented in C++/Cython and release the GIL, enabling true thread-level concurrency without the fork-safety issues that cause deadlocks on macOS (Apple Accelerate's internal mutex state is not fork-safe).
* Brute-force NN index (sklearn, Manhattan metric): built once on the cf-class instances and shared across threads (no serialisation overhead).  Manhattan distance is computed via SciPy cdist (Cython), which also releases the GIL. Brute-force is faster than BallTree at high dimensions (147 encoded columns) due to the curse of dimensionality.
* Boundary selection (k=1 query): uses n_jobs=-1 for multi-core parallelism via joblib (loky backend, spawn-like: safe after BLAS/Accelerate usage).
* Batch predict: for each boundary instance the relevance check builds a batch of all modified instances (one per differing feature, across all k counterfactuals) and calls model.predict() once, instead of one call per feature per counterfactual.  Batch array is pre-allocated with np.empty() instead of building a list and converting.
...
def _compute_default_workers() -> int:
    ...
    Reserves 2 logical CPUs for the OS and the main process, caps at 14 to avoid diminishing returns from CatBoost's internal thread pools.
    """
...
# Ordered category definitions
#
# Imported from constants.py to avoid duplication: if category labels or orderings change in constants.py this dict updates automatically.
# -----------------------------------------------------------------------------
...
def build_rank_maps(df: pd.DataFrame) -> dict[str, dict[str, int]]:
    ...
    Ordinal columns are those listed in ORDERED_CATEGORIES; they use the declared semantic order (rank 1 = lowest).  Ranks are 1-based and must
    be built from training data only.

    Nominal columns are NOT included here: they are handled by build_nominal_maps / encode_hybrid.
    """
...
def encode_hybrid(
    ...
    """
    Encode a categorical DataFrame into a float32 matrix where every column contributes values in [0, 1], making ordinal and nominal distances
    commensurable.

    Ordinal columns (in rank_maps)
    ------------------------------
    Rank-based encoding normalised per-column with min-max to [0, 1]: encoded = (rank - 1) / (max_rank - 1)   if max_rank > 1, else 0. Unknown values fall back to 0.5 (mid-range neutral).

    Nominal columns (in nominal_maps)
    ----------------------------------
    One-hot encoding, one column per category.  Within a single original column the raw Hamming distance between two samples is either 0 (same
    category) or 2 (different category: exactly two bits differ). Dividing by 2 gives a per-column distance of 0 or 1.  This is implemented by encoding each one-hot bit as 0.0 or 0.5, so that Manhattan distance on the encoded values recovers Hamming/2:
    |0.5 - 0.0| + |0.0 - 0.5| = 1.0  (= Hamming 2 / 2)

    Column order
    ------------
    Ordinal columns appear in feature_cols order; nominal one-hot columns are appended immediately after their source column, in category sort order. The encoded_col_names helper mirrors this order.
    ...
    # Pre-allocate output: one contiguous block instead of many small arrays concatenated at the end. Reduces allocation overhead and improves cache locality on large datasets (1M+ rows * 150+ encoded columns).
    ...
    for col in feature_cols:
        if col in rank_maps:
            ...
            # Vectorised via pd.Series.map(dict) (C-optimised in pandas) + fillna for unknown values, avoiding a Python-level lambda per row.
            rmap = rank_maps[col]
...
def encoded_col_names(
    ...
    Useful for debugging / logging.  Ordinal columns keep their name; nominal columns become 'COL=cat' strings.
    """
    names: list[str] = []
    for col in feature_cols:
        if col in rank_maps:
...
def n_encoded_cols(
    ...
    Used by encode_hybrid() to pre-allocate the output array, and available for diagnostics.
    """
    total = 0
    for col in feature_cols:
        if col in rank_maps:
...
def build_nn_index(X_enc: np.ndarray) -> NearestNeighbors:
    ...
    Uses sklearn NearestNeighbors with algorithm='brute' and Manhattan metric.  At high dimensions (e.g. 147 encoded columns), brute-force
    distance computation is faster than tree-based methods (BallTree/KDTree) which degrade due to the curse of dimensionality.

    n_jobs=1 because this index is shared across ThreadPoolExecutor workers: each thread calls kneighbors() concurrently, and the underlying SciPy cdist (Cython) releases the GIL, enabling true thread-level parallelism. Using n_jobs > 1 here would cause over-subscription (joblib threads inside ThreadPoolExecutor threads).

    Hybrid distance:
    dist(a, b) = sum_i |enc_i(a) - enc_i(b)|
...
def select_boundary_instances(
    ...
    1. Separate instances by TRUE label (y_true), not model predictions, so that the class pools are stable and independent of the model.
    2. Keep only orig-class instances that the model ALSO predicts as original_class (i.e. correctly classified). Misclassified instances
    would lie on the wrong side of the decision boundary and their counterfactual direction would be semantically inverted.
    3. Select boundary instances as those whose distance to the nearest true-cf-class neighbour is <= the percentile_th-th percentile. (Original authors use <=; we align with that convention.)

    The NN index is built on the counterfactual-class instances and returned so that worker processes can reuse it for the k-NN step (Algorithm 1).
    ...
    # Euclidean distance, Apple Accelerate is not involved.  n_jobs=-1 enables joblib parallelism: sklearn splits query points across multiple workers, each computing a subset of pairwise distances.
    # This is safe here because joblib uses the "loky" backend (spawn-like), not raw fork.
    logger.info(
...
def _process_boundary_chunk(
    ...
    For each boundary instance:
      1. Find the k nearest neighbours from the counterfactual class that differ in at least one feature (distance > 0).  The NN index is queried once for the ENTIRE chunk in a single batched call, then results are filtered per-instance to skip cross-class duplicates (identical features, different label).  This guarantees every counterfactual has >= 1 feature to substitute.
      2. Build a batch of all modified instances needed for the relevance check across all k counterfactuals, then call model.predict() once on the entire batch (batched Algorithm 2).
      3. For each CF independently, record which features are relevant (per-CF tracking for the new BoCSoR index).
      4. Take the UNION of relevant features across all k counterfactuals and emit one itemset row per boundary instance (input for ARM).

    Called from ThreadPoolExecutor - model, NN index and encoded arrays are shared in-process (no serialisation overhead).

    Parameters
    ----------
    precomputed_dists: If provided, skip the NN query and use these precomputed distances (shape: chunk_size * k_query).
    precomputed_tops: Corresponding NN indices for precomputed_dists.

    Returns
    -------
    (importance_counts, itemset_rows, distance_rows, label_relevance, value_relevance, n_cf_tested)
    """
    ...
    # When precomputed_dists/precomputed_tops are provided (multi-k mode), the expensive NN query was already performed once in run_bocsor_multi_k and is reused across all k values -- avoiding redundant O(N*M) distance computations.  Otherwise, query on-the-fly (single-k standalone mode).
    ...
    for row_i, b_idx in enumerate(chunk_indices):
        ...
        # Vectorised n_diff: numpy element-wise comparison instead of a Python-level generator expression (avoids per-element overhead).
        ...
        # Build one modified instance per (counterfactual, differing_feature) pair, collect their (cf_idx, fi) metadata, then call predict once.
        # Pre-count differing features to pre-allocate the batch array, avoiding intermediate list + costly np.array(list, dtype=object).
        ...
        # Derived from per_cf_relevant (already computed above) instead of re-iterating over all (pred, batch_meta) pairs a second time.
        relevant_union: set[str] = set()
        for feats in per_cf_relevant.values():
            relevant_union |= feats

        if relevant_union:
            # One row per boundary instance -- all relevant features on the same record. "itemset" is a space-separated string of FEATURE=value tokens (sorted for reproducibility), ready for ARM.
            items_str = " ".join(
...
def run_bocsor_single_k(
    ...
    Parameters
    ----------
    precomputed_chunks : If provided, reuse these chunk index lists instead of recomputing them (shared across all k values).
    precomputed_nn: If provided, a list of (dists, tops) arrays, one per chunk, from a single NN query with the maximum k_query.  Avoids redundant NN computation across multiple k values.
    ...
    """
    logger.info(
        "  k=%d | boundary instances: %d (class %d)", k, len(boundary_indices), original_class,
    )
    if len(boundary_indices) == 0:
        ...
        return (
            ...
            pd.DataFrame(columns=["instance_index", "cf_index", "k_neighbour_rank", "distance", "n_diff_features"]),
            {"k": k, "boundary_instances": 0, "instances_with_cf": 0, "instances_filtered_dist0": 0, "pct_filtered": 0.0, "instances_with_relevant_features": 0, "total_cf_tested": 0},
            empty_label,
            empty_value,
        )

    # -- Chunk and dispatch ----------------------------------------------------
    # When precomputed_chunks is provided (multi-k mode) the chunking was already computed once in run_bocsor_multi_k; reuse it.
    ...
    # ThreadPoolExecutor instead of ProcessPoolExecutor: CatBoost.predict() and MLPClassifier.predict() are implemented in C++/Cython and release the GIL, enabling true thread-level concurrency.  ProcessPoolExecutor with fork causes deadlocks on macOS because Apple Accelerate's internal mutex state is duplicated in an inconsistent state by fork().
    with ThreadPoolExecutor(max_workers=n_chunks) as executor:
        futures = {}
        for ci, chunk in enumerate(chunks):
            # When precomputed NN data is available, pass the chunk's (dists, tops) arrays so the worker skips the NN query.
    ...
    feat_imp = (
        pd.Series(total_importance, name="BoCSoR_importance") / max(n_with_cf, 1)
    ).sort_values(ascending=False)

    # -- New BoCSoR index: per-CF label-level ------------------------------
    # BoCSoR_k(LABEL) = times LABEL was relevant across all (instance, CF) pairs / total CFs tested.
    label_imp = (
        pd.Series(total_label_rel, name="BoCSoR_label") / max(total_cf_tested, 1)
    ).sort_values(ascending=False)

    # -- New BoCSoR index: per-CF value-level ------------------------------
    # BoCSoR_k(LABEL=value) = times that specific value was relevant / total CFs tested.
    value_imp = (
        pd.Series(total_value_rel, name="BoCSoR_value") / max(total_cf_tested, 1)
    ).sort_values(ascending=False)
...
def run_bocsor_multi_k(
...
) -> tuple[pd.DataFrame, pd.DataFrame, dict[int, pd.DataFrame], pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Run BoCSoR once per value in k_values on the training set.

    Key optimisations vs. the naive loop:
    - Hybrid encoding of X_train: computed once, reused for all k values. Ordinal columns: rank-based, min-max normalised to [0,1]. Nominal columns: one-hot / 2 (Hamming/2), each bit in {0.0, 0.5}.
    - Boundary selection via brute-force NN: multi-core distance computation (n_jobs=-1 via joblib), efficient at 147 encoded dimensions. Pools separated by TRUE label; only correctly-classified orig-class instances are candidates.  Built once, reused for all k values.
    - k-NN (Algorithm 1) in workers: batched brute-force query -- one call per chunk instead of one per boundary instance.
    - Workers run in ThreadPoolExecutor (GIL released by SciPy cdist and CatBoost/MLP predict); model and NN index shared in-process.
    - If zero boundary instances are found, all k values are skipped immediately without any tree query per k.
    - Relevance check uses batched model.predict() instead of one call per (counterfactual, feature) pair.

    Returns
    -------
    (all_itemsets_df, importance_df, per_k_itemsets, distances_df, filter_stats_df, label_imp_df, value_imp_df)
    ...
    distances_df: columns [k_value, instance_index, cf_index, k_neighbour_rank, distance, n_diff_features].
    ...
    if len(boundary_indices) == 0:
        ...
        empty_dist = pd.DataFrame(
            columns=["k_value", "instance_index", "cf_index", "k_neighbour_rank", "distance", "n_diff_features"]
        )
        empty_fstats = pd.DataFrame(
            columns=["k", "boundary_instances", "instances_with_cf", "instances_filtered_dist0", "pct_filtered","instances_with_relevant_features", "total_cf_tested"]
        )
    ...
    # The k-NN query is the most expensive step per chunk. Because the top-k_1 neighbours are always a prefix of the top-k_2 results (k_1 < k_2), a single query with k_query sized for the MAXIMUM k suffices for all k values. Each per-k run then simply slices the first k valid (dist > 0) neighbours from the same precomputed arrays. This eliminates len(k_values)-1 redundant O(N * M) brute-force distance computations.
    n_chunks   = min(n_workers, len(boundary_indices))
    chunk_size = math.ceil(len(boundary_indices) / n_chunks)
    ...
    for k in k_values:
        ...
        # Always tag with k_value; even if empty, so pd.concat produces a consistent column schema across all k values.
    ...
    return (combined, imp_df, per_k_itemsets, distances_df, filter_stats_df, label_imp_df, value_imp_df)
...
def load_split_data(
    ...
    Pre-split files are preferred.  A single dataset file is split internally with a warning recommending stage-1 regeneration.
    """
...
def train_catboost(
    ...
    - It accepts raw string-valued categorical columns with no manual encoding, avoiding the dimensionality explosion of one-hot encoding on columns such as OCCP (23 values) or POBP (80+ values).
    - Its ordered target statistics handle high-cardinality categories robustly without introducing target leakage.
    - Alfeo et al. (2023) found CatBoost to be the most accurate of the three classifiers they tested on the HCP benchmark tasks.

    Parameters
    ----------
    early_stopping_rounds : If set, training stops when the validation loss does not improve for this many consecutive rounds. An internal 80/20 stratified split is used as the eval set.  None = disabled (train for all iterations).
    """
    model = CatBoostClassifier(
    ...
    if early_stopping_rounds is not None:
        # Hold out 20% of training data as an internal validation set so CatBoost can monitor the eval metric and stop early.
        #
        # Note for BoCSoR: y_pred_train is computed on all of X_train (including this 20% eval split) after training completes, so boundary instances are selected from the full training set. The 20% eval rows are technically slightly less "known" to the model than the 80% training rows, but this is a minor effect and standard practice for early-stopped gradient boosting.
...
class MLPWrapper:
    ...
    MLPClassifier requires numeric inputs, so categorical features are integer-encoded: ordinal columns use their semantic rank from rank_maps;
    nominal columns use 1-based lexicographic rank from nominal_maps. Features are then standardised (zero-mean, unit-variance) using statistics
    computed at fit time, stored in the wrapper so predict() can apply the same transform.

    The CatBoost-specific ``thread_count`` keyword argument is silently dropped so that _process_boundary_chunk can call model.predict() with
    ...
    def __init__(
        ...
        self._mean = mean
        self._std = std
        # Build a combined int-encoding map: ordinal columns use their semantic rank; nominal columns use 1-based lexicographic index.
    ...
    def _encode(self, X: np.ndarray) -> np.ndarray:
        ...
        Unknown category values are mapped to 0. If standardisation statistics are available the output is standardised.

        Uses pd.Series.map(dict) for arrays with > 1000 rows (C-optimised dict lookup in pandas) and falls back to list comprehension for small batches where pandas overhead would dominate.
        """
        n_rows = X.shape[0]
    ...
    def predict(self, X, **kwargs) -> np.ndarray:
        ...
        Accepts either a pandas DataFrame or a numpy object array of raw string category values.
        """
...
def train_mlp(
    ...
    """
    Train an sklearn MLPClassifier on the (fully categorical) training set and return it wrapped in an MLPWrapper for a uniform predict() interface.

    The MLP provides a fundamentally different decision boundary geometry compared to tree-based classifiers (smooth non-linear surfaces vs.
    axis-aligned splits), making it an ideal complement to CatBoost for verifying that BoCSoR results are model-agnostic.
    ...
    early_stopping_rounds: Stop if validation loss does not improve for N consecutive rounds.  An internal 10% stratified split is used as the eval set. None = disabled.
    """
...
def train_model(
    ...
    rank_maps: Ordinal-only rank maps from build_rank_maps(). Required for MLP encoding; unused for CatBoost.
    nominal_maps : Nominal category lists from build_nominal_maps(). Required for MLP encoding; unused for CatBoost.
...
Examples:
  ...
  python -m src.feature_importance \\
    ...
    inp.add_argument("--dataset", type=Path, default=None, metavar="CSV", help="Single unsplit CSV (prefer --train/--test).")
    inp.add_argument("--split-size", type=float, default=0.2, metavar="FRACTION", help="Test fraction when --dataset is used.  Default: 0.2.")
...
def main() -> None:
    ...
    # Determine which classes still need to be processed.  For each requested class, check whether feature_importance[_classN].csv already exists. If all requested outputs are present, skip stage 2 entirely.
    ...
    # Deduplicate and sort the requested classes; for each, the counterfactual class is the other one (binary classification: 0 <-> 1).
    ...
    for orig_cls in orig_classes:
        cf_cls = 1 - orig_cls
        # suffix_map is keyed on orig_classes_requested (the full set), so it is correct even when only a subset of classes is processed.
...
def plot_distance_histograms(
    ...
    Uses the max-k run data (which contains ranks 1 through max_k) to show how distance and feature differences grow from the 1st to the k-th nearest counterfactual.
    ...
    Produces 3 types of PNG:
      ...
      - Stacked percentage bars: % of counterfactuals with 1, 2, 3, ... differing features, one bar per rank.

    Parameters
    ----------
    distances_df: DataFrame with columns [k_value, instance_index, cf_index, k_neighbour_rank, distance, n_diff_features].
    ...
    for rank in ranks:
        ...
        if has_ndiff and len(diff_values) > 1:
            ...
            ax.hist(layers, bins=bin_edges, stacked=True, edgecolor="black", linewidth=0.4, alpha=0.85, color=_colors[:len(layers)], label=labels)
            ax.legend(fontsize=10, title="Diff. features")
        else:
            ax.hist(dists, bins=bin_edges, edgecolor="black", alpha=0.75, color="#4C72B0")

        ax.axvline(float(np.median(dists)), color="red", linestyle="--", linewidth=1, label=f"median = {np.median(dists):.3f}")
            ...
            if has_ndiff and len(diff_values) > 1:
                ...
                for d in diff_values:
                    ...
                    if len(layer) > 0:
                        layers.append(layer)
                        labels.append(f"{d}f")
                ax.hist(layers, bins=bin_edges, stacked=True, edgecolor="black", linewidth=0.3, alpha=0.85, color=_colors[:len(layers)], label=labels)
...
def _write_summary(
    ...
    # Group summary_data by direction (each direction appears once in summary_data with its importance_df; we de-duplicate).
